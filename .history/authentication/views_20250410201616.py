...
def step_one_handler(request, user):
    ...
    if request.method == 'POST':
        if form.is_valid():
            ...
            application.save()
            return redirect(reverse('authentication:application_view') + '?step=2')
...
def step_two_handler(request, application):
    ...
    if request.method == 'POST' and form.is_valid():
        form.save()
        return redirect(reverse('authentication:application_view') + '?step=3')
...
def step_three_handler(request, application):
    ...
    if request.method == 'POST' and form.is_valid():
        documents = form.save()
        return redirect(reverse('authentication:application_view') + '?step=4')
...
def step_four_handler(request, application):
    ...
    if request.method == 'POST' and form.is_valid():
        form.save()
        return redirect(reverse('authentication:application_view') + '?step=5')
...
def step_seven_handler(request, application):
    ...
    if request.method == 'POST' and formset.is_valid():
        ...
        for instance in instances:
            ...
            instance.save()
        return redirect(reverse('authentication:application_view') + '?step=8')
...
def payment_handler(request, application):
    ...
    if request.method == 'POST':
        ...
        payment = paynow.create_payment('Application Fee', '1timsatande7@gmail.com')
        payment.add('Application Fee Payment', 20)

        if payment_method == 'mobile':
            response = paynow.send_mobile(payment, '0786167159', 'ecocash')
