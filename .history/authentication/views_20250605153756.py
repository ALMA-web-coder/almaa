...
def masters_application(request):
    # Helper to get or create the user's application
    ...
    if request.method == "POST":
        # Bind forms with POST data, pass existing instances if any
        ...
        if forms_valid:
            ...
            # Save related models
            def save_related(form, instance):
                obj = form.save(commit=False)
                obj.application = app
                obj.save()
                return obj

            save_related(contact_form, contact_instance)
            save_related(educational_form, educational_instance)
            save_related(work_experience_form, work_experience_instance)
            save_related(other_form, other_instance)
            save_related(document_form, documents_instance)
        ...
        # GET: populate with existing data if any
