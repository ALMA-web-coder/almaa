...
def masters_application(request):
    ...
    if request.method == "POST":
        ...
        # Adjust 'documents' to your actual related name
        documents_instance = getattr(application, 'documents', None) if application_exists else None
        documents_upload_form = DocumentUploadForm(
            request.POST, request.FILES,
            instance=documents_instance
        )

        contact_instance = getattr(application, 'contacts', None)
        if hasattr(contact_instance, 'all'):
            contact_instance = contact_instance.first()
        ...
        if hasattr(educational_instance, 'all'):
            educational_instance = educational_instance.first()
        ...
        masters_instance = getattr(application, 'certificate_edu', None)
        if hasattr(masters_instance, 'all'):
            masters_instance = masters_instance.first()

        document_upload_form = MastersEducationalDocumentForm(
            request.POST, request.FILES,
            instance=masters_instance
        )

        work_experience_qs = getattr(application, 'work_experiences', None)
        if hasattr(work_experience_qs, 'first'):
        ...
        if hasattr(other_instance, 'all'):
            other_instance = other_instance.first()
        ...
        if forms_valid:
            # Save or update the main application
