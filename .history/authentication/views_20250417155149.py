...
def check_payment_status(paynow, response, application):
    ...
    print(f"Payment Status: {status.paid}")  # Debugging line
    
    # If the status indicates that it's paid or testing success
    if status.paid or (status.message and 'TESTING: Faked Success' in status.message):
        application.is_paid = True
        try:
            application.save()
            ...
            print(f"Error saving application status: {e}")
