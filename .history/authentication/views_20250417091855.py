...
def step_two_handler(request, application):
    # Create a form instance, using data from previous session if available
    initial_data = request.session.get('personal_info', {})  # Load previous data if exists
    form = PersonalInfoForm(request.POST or None, initial=initial_data, instance=application)
