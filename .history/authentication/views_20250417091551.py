...
def step_two_handler(request, user):
    # Check if application info is in the session
    application_type = request.session.get('application_type')
    
    # If application_type exists in the session, initialize your form accordingly 
    application = Application.objects.filter(user=user).last()
    if application is None:
        return redirect(reverse('application_view') + '?step=1')

    form_data = request.session.get('personal_info', {})

    # Load the form with initial data if it exists
    form = PersonalInfoForm(request.POST or None, initial=form_data, instance=application)
