...
def payment_handler(request, application):
    if request.method == 'POST':
        ...
        # Get the Paynow payment URL
        response = paynow.send(payment)  # Use `send` if you want to generate a payment link
       
        # Check if the payment response is successful
        if response.success:
            # Redirect to Paynow payment link
            paynow_url = "https://www.paynow.co.zw/alma?"  # Replace with dynamic URL if needed
            return redirect(paynow_url)
        else:
            # Handle error gracefully
            error_message = getattr(response, 'message', "Payment failed with an unknown error.")
