...
def test_application_view_step_six_post(logged_in_client, user):
    ...
    transcript = SimpleUploadedFile(
        name='transcript.pdf',
        content=b'This is a test file for the transcript',
        content_type='application/pdf'
    )
    
    degree_certificate = SimpleUploadedFile(
        name='degree_certificate.pdf',
        content=b'This is a test file for the degree certificate',
        content_type='application/pdf'
    )
    
    CV = SimpleUploadedFile(
        name='CV.pdf',
        content=b'This is a test file for the CV',
        content_type='application/pdf'
    )
    a_level_certificate = SimpleUploadedFile(
        name='a_level_certificate.pdf',
        content=b'This is a test file for the A Level certificate',
        content_type='application/pdf'
    )
    
    o_level_certificate = SimpleUploadedFile(
        name='o_level_certificate.pdf',
        content=b'This is a test file for the O Level certificate',
        content_type='application/pdf'
    )
...
def test_application_view_step_seven_post(logged_in_client, user):
    logged_in_client.post(reverse('application_view') + '?step=1', data={'application_type': 'masters'})
    logged_in_client.post(reverse('application_view') + '?step=2', data={}) # Personal Info
    logged_in_client.post(reverse('application_view') + '?step=4', data={})  # Contact Info         
    logged_in_client.post(reverse('application_view') + '?step=5', data={})
    logged_in_client.post(reverse('application_view') + '?step=6', data={})
    
    form_data = {
          'employer': 'Tech Corp',
            'work_details': 'Software Engineer',
            'work_experience_from': '2015-01-01',
            'work_experience_to': '2020-01-01',
            'reference_1_name':'Tim Guinevere',
            'reference_1_email': 't@gmail.com',
            'reference_1_phone_number': '1234567890',
            'reference_2_name': 'Guinevere Tim',
            'reference_2_email':'g@gmail.com',
            'reference_2_phone_number': '0987654321',
    }
    
    response = logged_in_client.post(reverse('application_view') + '?step=7', data=form_data, follow=True)
    assert response.status_code == 302
    assert response.url == reverse('application_view') + '?step=8'
    # Check if the WorkExperience object was created
    application = Application.objects.get(user=user)
    assert application.work_experiences.count() == 1
    work_experience = application.work_experiences.first()
    assert work_experience.employer == form_data['employer']
    assert work_experience.work_details == form_data['work_details']
    assert work_experience.work_experience_from.strftime('%Y-%m-%d') == form_data['work_experience_from']