...
def test_application_view_step_five_post(logged_in_client, user):
    ...
    form_data = {
            'period_of_study':'2015-2020',
            'awarding_institution': 'CUT',
            'degree_diploma_level_classification': 'degree',
            'major_subjects': 'python',
...
def test_application_view_step_six_post(logged_in_client, user):
    # Create application and submit previous steps
    logged_in_client.post(reverse('application_view') + '?step=1', data={'application_type': 'masters'})
    logged_in_client.post(reverse('application_view') + '?step=2', data={})
    logged_in_client.post(reverse('application_view') + '?step=4', data={}) 
    logged_in_client.post(reverse('application_view') + '?step=5', data={})
    
    # Prepare the uploaded files
    transcript = SimpleUploadedFile(name='transcript.pdf', content=b'This is a test file for the transcript', content_type='application/pdf')
    degree_certificate = SimpleUploadedFile(name='degree_certificate.pdf', content=b'This is a test file for the degree certificate', content_type='application/pdf')
    CV = SimpleUploadedFile(name='CV.pdf', content=b'This is a test file for the CV', content_type='application/pdf')
    a_level_certificate = SimpleUploadedFile(name='a_level_certificate.pdf', content=b'This is a test file for the A Level certificate', content_type='application/pdf')
    o_level_certificate = SimpleUploadedFile(name='o_level_certificate.pdf', content=b'This is a test file for the O Level certificate', content_type='application/pdf')
    
    form_data = {
        'transcript': transcript,
        'degree_certificate': degree_certificate,
        'CV': CV,
        'a_level_certificate': a_level_certificate,
        'o_level_certificate': o_level_certificate,
    }

    # Make the POST request
    response = logged_in_client.post(reverse('application_view') + '?step=6', data=form_data, follow=True)

    # Check assertions with debug information
    assert response.status_code == 302, f"Expected 302 redirect, but got {response.status_code}. Response content: {response.content}"
    assert response.url == reverse('application_view') + '?step=7', f"Expected redirect to {reverse('application_view')}?step=7, but got {response.url}."

    # Optional: Add additional assertions for more thorough debugging
    application = Application.objects.get(user=user)  # Assuming application is linked to the logged-in user
    assert application.masters.exists(), "No masters documents were found linked to the application."
    
    # Fetch the uploaded documents
    uploaded_documents = application.masters.first()
    
    # Check for specific files
    assert uploaded_documents.transcript.name == 'transcript.pdf', f"Expected transcript to be uploaded, but got {uploaded_documents.transcript.name}."
    assert uploaded_documents.degree_certificate.name == 'degree_certificate.pdf', f"Expected degree_certificate to be uploaded, but got {uploaded_documents.degree_certificate.name}."
    assert uploaded_documents.CV.name == 'CV.pdf', f"Expected CV to be uploaded, but got {uploaded_documents.CV.name}."
    assert uploaded_documents.a_level_certificate.name == 'a_level_certificate.pdf', f"Expected a_level_certificate to be uploaded, but got {uploaded_documents.a_level_certificate.name}."
    assert uploaded_documents.o_level_certificate.name == 'o_level_certificate.pdf', f"Expected o_level_certificate to be uploaded, but got {uploaded_documents.o_level_certificate.name}."