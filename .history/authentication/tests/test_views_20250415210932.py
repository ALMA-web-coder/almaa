...
def test_application_view_step_three_post(logged_in_client, user):
    ...
    logged_in_client.post(reverse('application_view') + '?step=1', data={'application_type': 'masters'})
    form_data = {
        'id_document': 'path/to/id_document.pdf',  
        'birth_certificate': 'path/to/birth_certificate.pdf',
    }
    
    # Step 3: Upload documents
    response = logged_in_client.post(reverse('application_view') + '?step=3', data=form_data)
    assert response.status_code == 302
...
def test_application_view_step_four_post(logged_in_client, user):
    ...
    form_data = {
        # Populate with necessary contact fields
    }
