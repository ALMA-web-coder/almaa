...
def test_application_view_step_one_get(logged_in_client):
    response = logged_in_client.get(reverse('application_view') + '?step=1')
    assert response.status_code == 200
    assert 'application/program_selection.html'
