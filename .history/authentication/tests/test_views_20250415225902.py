...
def test_application_view_step_six_post(logged_in_client, user):
    ...
    form_data = {
        'transcript': SimpleUploadedFile(
            name='transcript.pdf', 
            content=b'Test file content for transcript', 
            content_type='application/pdf'
        ),
        'degree_certificate': SimpleUploadedFile(
            name='degree_certificate.pdf', 
            content=b'Test file content for degree certificate', 
            content_type='application/pdf'
        ),
        'CV': SimpleUploadedFile(
            name='CV.pdf', 
            content=b'Test file content for CV', 
            content_type='application/pdf'
        ),
        'a_level_certificate': SimpleUploadedFile(
            name='a_level_certificate.pdf',
            content=b'Test file content for A-level certificate',
            content_type='application/pdf'
        ),
        'o_level_certificate': SimpleUploadedFile(
            name='o_level_certificate.pdf',
            content=b'Test file content for O-level certificate',
            content_type='application/pdf'
        ),
    }
    ...
    assert response.status_code == 302, f"Expected 302 redirect, but got {response.status_code}. Response content: {response.content.decode()}"
