...
def check_payment_status(paynow, response, application):
    ...
    print(f"Payment Status: Paid - {status.paid}")  # Debugging line

    # Check if the payment is marked as paid
    if status.paid:  # Only check the 'paid' status
        application.is_paid = True
        try:
            application.save()
            print("Application status updated to paid.")
            return True  # Return True if the application is successfully updated
        except Exception as e:
            print(f"An error occurred while updating the application status: {e}")
            return False  # Return False if there was an issue saving the application
    else:
        print("Payment not paid yet or failed.")
        return False  # Return False if the payment is not successful
