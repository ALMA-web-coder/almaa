...
from django.urls import reverse
from .forms import ProgramTypeForm, PersonalInfoForm, DocumentUploadForm, ContactAndAddressForm, EducationalBackgroundMastersForm, EducationalBackgroundCertificateForm, MastersEducationalDocumentForm, CertificateEducationalDocumentForm, WorkExperienceForm, OtherInfoForm
from .models import Application, Documents, Contact, WorkExperience, Other
from paynow import Paynow  # Assuming you have Paynow integrated
...
def application_view(request):
    ...
    # Retrieve the user's most recent application
    ...
    if application.is_paid and application.is_finalized:
        return step_nine_handler(request, application)  # Direct to success page
...
def step_one_handler(request, user):
    form = ProgramTypeForm(request.POST or None)
    if request.method == 'POST' and form.is_valid():
        application = Application(user=user, application_type=form.cleaned_data['application_type'])
        application.save()  # Save the initial application object
...
def step_two_handler(request, application):
    ...
    if request.method == 'POST' and form.is_valid():
        form.save(commit=False)  # Set instance data but don't save yet
...
def step_three_handler(request, application):
    ...
    if request.method == 'POST' and form.is_valid():
        form.save(commit=False)  # Set data but don't save yet
...
def step_five_handler(request, application):
    ...
    # Determine the template based on application type
    ...
    # Get the existing educational instance if exists
    existing_instance = application.educational.first() if application.educational.exists() else None
    
    # Initialize the form with request data or existing instance
    form = form_class(request.POST or None, instance=existing_instance)
    
    if request.method == 'POST' and form.is_valid():
        educational = form.save(commit=False)  # Don't save to the database yet
        educational.application = application  # Link to the application
        educational.save()  # Save the educational record
...
def step_six_handler(request, application):
    form_class = MastersEducationalDocumentForm if application.application_type == 'masters' else CertificateEducationalDocumentForm
    ...
    if request.method == 'POST' and form.is_valid():
        ...
        educational.application = application  # Link to application
        educational.save()  # Save on last step
        return redirect(reverse('application_view') + '?step=7')
    return render(request, 'application/educational_document.html', {'form': form})
...
def step_seven_handler(request, application):
    ...
    if request.method == 'POST' and formset.is_valid():
        ...
        for instance in instances:
            instance.application = application  # Link to application
            instance.save()  # Save on last step
...
def payment_handler(request, application):
    ...
    if request.method == 'POST':
        payment_amount = 20  # Example amount, adjust as necessary
        payment = paynow.create_payment('Application Fee', 'moses.chundu@cccsea.org')
        payment.add('Application Fee Payment', payment_amount)

        payment_method = request.POST.get('payment_method')
        
        if payment_method == 'mobile':
            response = paynow.send_mobile(payment, '0771111111', 'ecocash')  # Example mobile number
            if response.success:
                payment_successful = check_payment_status(paynow, response, application)
                if payment_successful:
                    messages.success(request, 'Payment successful.')
                    application.is_paid = True
                    application.is_finalized = True
                    application.save()  # Save the application after payment
                    return redirect(reverse('application_view') + '?step=10')
                else:
            ...
            if response.success:
                if response.has_redirect:
                    messages.info(request, 'Payment pending. Please complete the payment.')
    ...
    return render(request, 'application/payment.html', {'application': application})
