...
def payment_handler(request, application):
    if request.method == 'POST':
        user_mobile_number = request.POST.get('phone')
        ...
        # Send to Paynow and get response
        response = paynow.send_mobile(payment, user_mobile_number, 'ecocash')

        # Inspect the response object
        print("Paynow response:", response)  # Debug line

        if response.success:
            # Redirect to the Paynow payment page
            redirect_url = response.redirect_url  # Check if redirect_url is present
            return redirect(redirect_url)
        else:
            # Handle the case where payment did not succeed
            error_message = response.message if hasattr(response, 'message') else "Payment failed with unknown error."
            messages.error(request, f"Failed to process payment: {error_message}")
