...
def payment_handler(request, application):
    ...
    if request.method == 'POST':
        ...
        # Redirect user directly to Paynow payment page
        response = paynow.send(payment)
        print(f"Payment Response: {response.__dict__}")
        ...
        if response.success:
            if response.has_redirect:
                ...
                messages.error(request, 'Payment didn\'t redirect. Please contact support.')
        else:
