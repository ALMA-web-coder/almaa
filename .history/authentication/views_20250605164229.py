...
def masters_application(request):
    ...
    if request.method == "POST":
        # Prepare related instances
        contact_instance = (
            application.contacts.first() if application_exists and application.contacts.exists() else None
        )
        educational_instance = (
            application.educational.first() if application_exists and application.educational.exists() else None
        )
        # do the same for other related models
        work_experience_instance = (
            application.work_experiences.first() if application_exists and application.work_experiences.exists() else None
        )
        other_instance = (
            application.other.first() if application_exists and application.other.exists() else None
        )
        ...
        documents_upload_form = DocumentUploadForm(
            ...
            instance=application.documents.first() if application.documents.exists() else None
        )
        ...
        document_upload_form = MastersEducationalDocumentForm(
            request.POST, request.FILES, 
            instance=application.documents if application_exists else None
        )
        ...
        # Validate all forms
        if all([
            # Add your validation checks here, e.g.,
            ...
            if application_exists:
                # application already exists, update if needed
                application = application
            else:
