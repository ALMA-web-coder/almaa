...
def application_view(request):
    ...
    if application.is_paid and application.is_finalized:
        return step_nine_handler(request, application)
...
def step_five_handler(request, application):
    form_class = (
        ...
        else EducationalBackgroundCertificateForm
    )
...
def payment_handler(request, application):
    ...
    if request.method == 'POST':
        payment_amount = 20  # Adjust as necessary
        ...
        try:
            if payment_method == 'mobile':
                response = paynow.send_mobile(payment, request.POST.get('mobile_number'), 'ecocash')
                if response.success:
