...
def payment_handler(request, application):
    if request.method == 'POST':
        user_mobile_number = request.POST.get('phone')  # Retrieve the user's mobile number
        ...
        payment = paynow.create_payment(f'Application Fee for {application}', user_email)

        # Adding payment item
        payment.add('Application Fee Payment', 20.00)
        
        try:
            # Sending mobile request
            response = paynow.send_mobile(payment, user_mobile_number, 'ecocash')

            if response.success:
                poll_url = response.poll_url  # Save the poll URL for future reference
                messages.info(request, "Payment instructions sent. Please check your mobile for details.")
                
                # You can save the poll_url to your database if needed for transaction status checking

                # Redirect after sending payment request
                return redirect(reverse('authentication:application_view') + '?step=9')
            else:
                # Handle unsuccessful response
                messages.error(request, f"Failed to process payment: {response.message}")

        except Exception as e:
            # Handle any exceptions during the Paynow API call
