...
def registrar_view(request, application_id):
    # Retrieve the application or return a 404 error
    application = get_object_or_404(Application, id=application_id)
    
    # Get the most recent status instance for the application
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save() 
            return redirect('application_detail', application_id=application_id)  # Redirect after successful update
    else:
        form = StatusForm(instance=status_instance)  # Prepare form with existing status

    return render(request, 'application/application_detail.html', {
        'form': form,
        'application': application,
        'status': status_instance.status,  # Current status
        'last_update': status_instance.updated_at  # Last updated timestamp
    })
