...
def step_one_handler(request, user):
    ...
    if request.method == 'POST':
        if form.is_valid():
            ...
            # Save the application type in the database
            application = Application(user=user)
            application.application_type = application_type
            application.save()

            # Save application type to session for resuming later
            request.session['application_type'] = application_type

            return redirect(reverse('application_view') + '?step=2')
        else:
            # Debugging output to see form errors
            print(form.errors)
    return render(request, 'application/program_selection.html', {'form': form})

def step_two_handler(request, application):
    # Load previous data if exists, ensuring dates are serialized
    personal_info = request.session.get('personal_info', {})
    
    # Convert dates to string if applicable
    if 'date_field' in personal_info and isinstance(personal_info['date_field'], date):
        personal_info['date_field'] = personal_info['date_field'].isoformat()

    form = PersonalInfoForm(request.POST or None, initial=personal_info, instance=application)

    if request.method == 'POST':
        if form.is_valid():
            # Save the cleaned data in the session to persist through steps
            cleaned_data = form.cleaned_data
            
            # Convert any date fields to string representations
            if 'date_field' in cleaned_data and isinstance(cleaned_data['date_field'], date):
                cleaned_data['date_field'] = cleaned_data['date_field'].isoformat()

            request.session['personal_info'] = cleaned_data

            # Save the form instance to the database
            form.save()
            return redirect(reverse('application_view') + '?step=3')  # Move to the next step
...
def payment_handler(request, application):
    ...
    return render(request, 'application/payment.html', {
        ...
        'amount': 20,  # Example amount, replace with the appropriate logic to retrieve it
    })
