...
def payment_handler(request, application):
    if request.method == 'POST':
        ...
        # Create a payment
        payment = paynow.create_payment(f'Application Fee for {application}', '')
        payment.add('Application Fee Payment', 20.00)

        # Generate the payment URL
        response = paynow.send(payment)  # This generates a payment link
        print(response)
       
        # Check if the payment response is successful
        if response.success:
            # Redirect users to the Paynow payment link
            return redirect(response.redirect_url)  # redirect_url generated by Paynow
        else:
    ...
    # This is where you render your payment page if not POST
